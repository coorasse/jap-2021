...
class FileReader
  ...
  def world
    ...
    streets_count.times.map do |i|
      ...
      street = Street.new(start: start_intersection_id, end: end_intersection_id, name: street_name, length: street_length, cars_count: 0)
    ...
    cars = cars_count.times.map do |i|
      ...
      street_names.each do |street_name|
        streets_hash[street_name].cars_count += 1
      end
      Car.new(streets: street_names.map { |street_name| streets_hash[street_name]})
    end
